...
public class PowerUpBall {
    ...
    private final double INITIAL_BALL_SPEED = 5;
    ...
    public void updateBalls() {
        ...
        for (Ball currentBall : balls) {
            ...
            if (ghostMode) {
                ...
                // Chế độ bình thường
                if (!ghostMode) {
                    if (checkAndHandleBrickCollisions(currentBall)) {
                    }
                }
            }
    ...
    private boolean checkAndHandleBrickCollisions(Ball ball) {
        for (Brick brick : brickManager.getBricks()) {
            if (!brick.isDestroyed() && brick.getType() != 1) {
                if (ball.checkCollisionWithBrick(brick)) {
                    handleBrickHit(brick);
                    return true; // Đã va chạm → bật rồi
                }
            }
        }
        return false;
    ...
    private void handleBrickHit(Brick brick) {
        switch (brick.getType()) {
            ...
            case 2: // Gạch cứng
                ...
                break;
            case 3: return; //gạch không thể phá
            default:
                brick.onHit();
                if (brick.isDestroyed()) {
